...
function updateEdgeDistance(){const f=$("#edgeForm"),from=graph.nodes.find(n=>n.code===f.elements.from_code.value),to=graph.nodes.find(n=>n.code===f.elements.to_code.value);f.elements.distance_meters.value=from&&to?SkyGateConnections.distanceBetween(from,to):"";}
function fillEdge() { const f=$("#edgeForm"); if(selectedEdge==null) { f.reset(); updateEdgeDistance(); return; } const e=graph.edges[selectedEdge]; Object.entries(e).forEach(([k,v])=>{if(f.elements[k]) f.elements[k].type==="checkbox"?f.elements[k].checked=!!v:f.elements[k].value=v??"";}); updateEdgeDistance(); }
...
$("#edgeForm").elements.from_code.onchange=updateEdgeDistance;
$("#edgeForm").elements.to_code.onchange=updateEdgeDistance;
$("#edgeForm").onsubmit=e=>{e.preventDefault();const f=e.currentTarget,distance=num(f.elements.distance_meters.value),previous=selectedEdge==null?{}:graph.edges[selectedEdge],n={...previous,from_code:f.elements.from_code.value,to_code:f.elements.to_code.value,edge_type:f.elements.edge_type.value,distance_meters:distance,walk_time_seconds:distance,instruction:previous.instruction??null,is_bidirectional:checked(f,"is_bidirectional"),is_accessible:checked(f,"is_accessible"),is_estimated:previous.is_estimated??true};const otherEdges=graph.edges.filter((_,index)=>index!==selectedEdge);if(n.from_code===n.to_code||SkyGateConnections.isDuplicate(otherEdges,n.from_code,n.to_code)||validate({...graph,edges:[n]}).some(x=>x.includes("Mudança")||x.includes("Peso"))){alert("Aresta inválida ou duplicada.");return;}if(selectedEdge==null){graph.edges.push(n);selectedEdge=graph.edges.length-1;}else graph.edges[selectedEdge]=n;save();};
